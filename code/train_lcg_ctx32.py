"""
LCG (glibc) 上下文窗口扩展实验：context_len 16 → 32

假设：
  - LCG (glibc) 的问题是"拓扑绞碎"（乘法取模把流形绞碎）
  - 这是一个非线性信息损失问题，而非线性不可逆性
  - 扩展上下文窗口不期望解决这个问题
  - 这是一个验证实验：确认即使有更多上下文信息，
    模型仍然无法克服拓扑结构的损毁

基于 train_learnability.py，只跑 lcg_glibc，context_len=32。
"""
...
def train(output_dir):
    ...
    exp_dir = os.path.join(output_dir, "lcg_glibc_ctx32")
    ...
    step = 0
    pbar = tqdm(total=CONFIG["total_steps"], desc="Training lcg_glibc ctx32")
    ...
    print(f"\n{'='*50}")
    print(f"Experiment: lcg_glibc context_len=32")

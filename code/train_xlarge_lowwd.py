#!/usr/bin/env python3
"""
第四阶段：10x 模型 + 低 weight decay

验证假设：weight decay 太强导致"学会了又忘了"

用法：
    python train_xlarge_lowwd.py
"""
...
CONFIG = {
    ...
    "num_heads": 16,
    "num_layers": 4,
    # 关键改动：大幅降低 weight decay
    "lr": 5e-4,
    "weight_decay": 0.01,       # 0.5 → 0.01 (50倍降低)
    "batch_size": 128,
    "total_steps": 100000,
    "eval_every": 500,
    "seed": 42,
...
class XLargeTransformer(nn.Module):
...
def train(seq_type, output_dir):
    ...
    print(f"Weight decay: {CONFIG['weight_decay']} (低)")

    torch.manual_seed(CONFIG["seed"])
    np.random.seed(CONFIG["seed"])

    exp_dir = os.path.join(output_dir, f"{seq_type}_xlarge_lowwd")
    ...
    step = 0
    pbar = tqdm(total=CONFIG["total_steps"], desc=f"{seq_type} (xlarge, low wd)")
    ...
    print(f"\n{'='*50}")
    print(f"{seq_type} (XLARGE, LOW WD)")
...
if __name__ == "__main__":
    # 两个都跑
    output_dir = "/workspace/ai-theorys-study/arxiv/wechat62/results"

    for seq in ["lfsr_31", "lcg_glibc"]:
        train(seq, output_dir)

"""
LFSR-31 上下文窗口扩展实验：context_len 16 → 32

假设：
  - LFSR-31 内部状态 31 bit
  - context_len=16 时，每步移 1 位，相邻步 7 bit 重叠，独立信息 ~24 bit < 31 bit → 50% 天花板
  - context_len=32 时，独立信息 ~39 bit > 31 bit → 应能突破 50%，接近 ~100%
...
def train(output_dir):
    ...
    exp_dir = os.path.join(output_dir, "lfsr_31_ctx32")
    ...
    step = 0
    pbar = tqdm(total=CONFIG["total_steps"], desc="Training lfsr_31 ctx32")

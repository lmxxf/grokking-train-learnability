...
Docker 容器：`magical_bhabha`（镜像 `nvcr.io/nvidia/pytorch:25.11-py3`）
...
# 第五阶段：lfsr_31 上下文窗口扩展（context_len 16→32）
python code/train_lfsr31_ctx32.py --output_dir results
```
...
- [ ] 扩大数据量：seq_length 从 10000 扩到 200000，验证数据量对边界序列的影响
  - 预测：lcg_glibc 照样学不会（拓扑粉碎），lfsr_31 可能从 50% 往上走（XOR 线性，更多样本可能多抓几个 bit）
- [ ] 扩大上下文窗口：lfsr_31 的 context_len 从 16 扩到 32，验证窗口大小对部分 Grokking 的影响
  - 当前 16 步 × 8 bit，独立信息约 24 bit < 31 bit 内部状态 → 信息不足
  - 32 步独立信息约 39 bit > 31 bit，理论上足够还原完整状态，可能突破 50%
